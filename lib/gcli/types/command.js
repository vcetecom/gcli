...
CommandType.prototype._addToPredictions = function(predictions, option, arg) {
  ...
  // If someone hasn't typed anything, we only show top level commands in the
  // menu. i.e. sub-commands (those with a space in their name) are excluded.
  // We do this to keep the list at an overview level.
  // If someone has typed something, then we exclude parent commands and
  // include only things that are actually executable. We do this because the
  // user is drilling down and doesn't need the summary level.
